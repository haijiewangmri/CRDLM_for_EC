...
def set_seed():
    ...
    torch.cuda.manual_seed_all(170)
...
def get_feature_bag_path(data_dir, slide_id):
    return os.path.join(data_dir, f"{slide_id}.csv")
...
class FeatureBagsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        slide_id = self.slide_df["CaseName"][idx]
        label = self.slide_df["label"][idx] - 1

        full_path = get_feature_bag_path(self.data_dir, slide_id)
        features = pd.read_csv(full_path).values
...
def run_train_eval_loop(
    ...
    writer = SummaryWriter(os.path.join("./runs_800", run_id))
...
def main(args):
    ...
    for l, (train_index, val_index) in enumerate(cv.split(df, df['label'].values)):
        fold_index = l
        real_index = df.index
        training_set = df.loc[real_index[train_index], :]
        validation_set = df.loc[real_index[val_index], :]
        ...
        for i, hps in enumerate(hparams_to_use):
            run_id = f"fold_{fold_index}_{hps['model_size']}_{hps['sampling_method']}_hp{i}"
            print(hps)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--manifest",
        type=str,
        default=r'E:\study\EC\subtype\20240521 DL模型\moco_feature\fudan292_DL.csv',
        help="CSV file listing all slides, their labels, and which split (train/test/val) they belong to.",
    )
    parser.add_argument(
        "--data_dir",
        type=str, default=r'E:\study\EC\subtype\20240521 DL模型\slice_feature_800\fudan292',
