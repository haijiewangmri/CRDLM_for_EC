...
class RadiomicsFeatureExtractor:
    ...
    def __GetFeatureValuesEachModality(self, data_path, roi_path, key_name):
        if self.__is_ignore_tolerence:
            ...
            # # image095 = np.percentile(image_array, 95)
            # # image_array = np.clip(image_array, image_array.min(), image095)
    ...
    def __IterateCase(self, root_folder, key_name_list, roi_key, show_key_name_list, store_path=''):
        case_name_list = os.listdir(root_folder)[:50]
        case_name_list.sort()
...
if __name__ == '__main__':
    ...
    extractor.Execute(r'.\image',
                      key_name_list=['image.nii.gz'],
                      roi_key=['roi.nii.gz'],
                      store_path=r'.\radiomcis feature.csv')
# t1ce,dwi,t1,t2
